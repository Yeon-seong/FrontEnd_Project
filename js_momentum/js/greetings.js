...
function onLoginSubmit(event) {
  ...
  localStorage.setItem(USERNAME_KEY, userName); // step4. userName값을 username이라는 key와 함께 local storeage에 저장한다
  paintGreetings();                             // step5. onLoginSubmit 함수에서는 유저 정보가 input으로부터 오고 있다
}
...
/*  유저 정보를 저장하는 savedUsername 변수를 선언하기
    ...
    userName이라는 key를 가지고 savedUsername을 통해 유저 정보 존재 여부를 찾는다  */
